...
def _call_solar(api_key, system, user, temperature=0.1):
    headers = {
    ...
    try:
        res = requests.post(SOLAR_URL, headers=headers, json=payload, timeout=20)
        res.raise_for_status()
        return res.json()["choices"][0]["message"]["content"].strip()
    except Exception as e:
        st.warning(f"Solar 호출 실패: {e}")
        return ""
...
def normalize_major(api_key, user_input, candidate_list):
    ...
    result = _call_solar(api_key, system, user)
...
def summarize_duty(api_key, teukgi_name, raw_desc=""):
    ...
    return _call_solar(api_key, system, user, temperature=0.5)
